// responsible for interacting with the ruby shim that invokes the customer labmda function

import { InvocationRequest } from 'amplify-function-plugin-interface/src';
import execa from 'execa';
import path from 'path';

const shimPath = path.join(__dirname, '../../shim/shim.rb');

export async function rubyInvoke(context: any, request: InvocationRequest): Promise<any> {
  ...
  const childProcess = execa('bundle', ['exec', 'ruby', shimPath, handlerFile + '.rb', handlerName]);
